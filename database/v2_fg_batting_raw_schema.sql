-- FanGraphs Batting Schema (Clean Column Names)

CREATE TABLE IF NOT EXISTS fg_batting_leaders (
    -- Core identifiers
    ...
    w_league NUMERIC(8,1),
    defense NUMERIC(8,1),
    offense NUMERIC(8,1),
    rar NUMERIC(8,1),
    war NUMERIC(6,1),
    warold NUMERIC(6,1),
    base_running NUMERIC(8,1),
    spd NUMERIC(6,1),
    w_rc_plus INTEGER,
    w_bs_r NUMERIC(8,1),
	...
	-- Primary key
	CONSTRAINT fg_batting_leaders_pkey PRIMARY KEY (playerid, season)
);

-- Indexes for performance
CREATE INDEX IF NOT EXISTS idx_fg_batting_leaders_season ON fg_batting_leaders(season);
CREATE INDEX IF NOT EXISTS idx_fg_batting_leaders_player ON fg_batting_leaders(playerid);
CREATE INDEX IF NOT EXISTS idx_fg_batting_leaders_war ON fg_batting_leaders(war DESC);
CREATE INDEX IF NOT EXISTS idx_fg_batting_leaders_wrc_plus ON fg_batting_leaders(w_rc_plus DESC);
CREATE INDEX IF NOT EXISTS idx_fg_batting_leaders_team ON fg_batting_leaders(team_name_abb);
CREATE INDEX IF NOT EXISTS idx_fg_batting_leaders_player_name ON fg_batting_leaders(player_name);

COMMENT ON TABLE fg_batting_leaders IS 'FanGraphs batting data with clean snake_case column names';